...
console.log('SISTEMA DE APOSTAS EUROMILHÕES');
console.log('='.repeat(60));

/**
 * POST /api/bet
 * Regista uma aposta EuroMilhões
 * 
 * Body:
 * {
 *   "accountId": "123456789",
  ...
  try {
    ...
    if (!accountId || !numbers || !stars) {
      return res.status(400).json({
        success: false,
        message: 'Dados incompletos. Forneça accountId, numbers e stars.'
      });
    }

    // Validar chave EuroMilhões
    if (numbers.length !== 5 || stars.length !== 2) {
      return res.status(400).json({
        success: false,
        message: 'Chave inválida. Deve ter 5 números e 2 estrelas.'
      });
    }

    // Validar números (1-50)
    if (numbers.some(n => n < 1 || n > 50)) {
      return res.status(400).json({
        success: false,
        message: 'Números devem estar entre 1 e 50.'
      });
    }
    ...
    // PASSO 1: Contactar CrediBank para obter cheque digital de 10 créditos
...
app.get('/api/health', (req, res) => {
  res.json({
    status: 'ok',
    server: 'EuroMilhões Betting System',
